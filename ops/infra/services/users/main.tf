...
resource "aws_db_user" "read_user" {
  db_instance_identifier = module.users_db.db_resource_id
  name = "readuser"
...
module "users_sg" {
  ...
  name = "${var.project_name}-users-sg"
  ...
  ingress_rules = [
    {
      ...
      description = "Allow access to port 8030"
    }
  ]
  egress_rules = [
    {
...
resource "aws_elasticache_cluster" "users_redis" {
  cluster_id      = "${var.project_name}-users-redis"
  engine          = "redis"
  node_type       = "cache.t3.micro"
  num_cache_nodes = 1
  port            = 6379
  subnet_group_names = [module.common.db_subnet_group_name]
  security_group_ids = [module.common.common_security_group_id, module.users_sg.security_group_id]
}
